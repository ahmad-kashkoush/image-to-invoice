...
|---|---|---|---|---|
| 1 | Image Extraction | `extraction/` | `vision_extractor.py` (Claude Haiku 4.5 vision pass; `currency` added to the schema/prompt/parser), `ocr_fallback.py` (deliberate no-op stub), `models.py`, `config.py`, `__init__.py::extract_order` | 0012 |
| 2 | Normalization & Validation | `normalization/` | `normalizer.py`, `validators.py`, `models.py`, `parsing.py`, `config.py`; `payment_details` carried through (was silently dropped); line-item price completeness check, discount/VAT/unit-price range checks, and real `currency` capture + ISO 4217 canonicalization added | 0001, 0011, 0012 |
...
9. **Task-spec gaps** (Order Date, currency comparison, address read-back,
   ...
   [README.md](README.md#next-steps). Currency is now genuinely captured
   end-to-end through extraction/normalization (`RawOrder.currency` →
   `NormalizedOrder.currency`, canonicalized to ISO 4217, ADR 0012) — an
   earlier version of this entry claimed this was already true and it
   wasn't (see `docs/implementation-notes.md`'s correction note); items 10-12
   below are what's still open for it and the other two.
...
11. **Currency has no UI-side verification.** Extraction/normalization now
    genuinely capture and canonicalize it (item 9, ADR 0012); but
    `verification/comparisons.py` has no `currency_equals`, and nothing reads
    a currency control off the order editor to compare it. Needs a live VM
    check first — it isn't confirmed Fakturama's order editor even exposes
    currency as a per-order control rather than a fixed per-installation
    setting; that decides whether this is a real check or a no-op.
12. **Addresses are normalized but never verified.** `NormalizedOrder.
    ...
    discrete fields before a comparator can be written.
